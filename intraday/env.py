...
class MultiAgentEnv(Exchange, gym.Env):
    ...
    def __init__(self,
        ...
        assert (max_trades is None or isinstance(max_trades, int))
    ...
    def reset(self,
              ...
              episode_max_duration: Optional[Union[Real, timedelta]] = timedelta(hours=5),
              n_agents: int = -1,
              keep_state=False,
    ...
    def step(self,
        ...
        rewards = []
        for account in self.accounts:
            partial_reward = self.reward_scheme.get_reward(env=self, account=account)

            if self.max_trades is not None and account.position != 0:
                current_trade = self.current_trades.get(account, 0)

                penalty_reward = 1.0 - float(self.max_trades - current_trade) / self.max_trades
                #partial_reward = partial_reward - partial_reward * penalty
                # account.balance -= penalty
                if penalty_reward >= 0.8:
                    partial_reward = -1 * penalty_reward * abs(partial_reward)
                    # print(f'Penalty reward {penalty_reward} {partial_reward}')

                self.current_trades[account] = current_trade + 1
        ...
        while True:
            try:
                ...
                if frame is not None:
                    ...
                    if (frame.time_end is not None) and \
                        (frame.time_end >= self.episode_start_datetime):
                        break
...
class SingleAgentEnv(MultiAgentEnv):
    ...
    def __init__(self,
        ...
        super(). __init__(
            ...
            agent_order_delay=agent_order_delay, broker_order_delay=broker_order_delay,
            order_luck=order_luck, commission=commission, idle_penalty=idle_penalty, max_trades=max_trades,
    ...
    def print_summary(self):
        print(f'Score: {self.score:0.2f}    \n' 
            f'Balance: {self.account.balance or np.nan:0.2f}   \n'
            f'Profit factor: PF: {self.account.report.profit_factor or np.nan:0.2f}  \n'
            f'Sortino Ratio: SR: {self.account.report.sortino_ratio or np.nan:0.2f}  \n'
            f'Gross profit: {self.account.report.gross_profit or np.nan:0.2f}  \n'
            f'Net profit: {self.account.report.net_profit or np.nan:0.2f}  \n'
            f'Max drawdown: {self.account.report.max_drawdown or np.nan:0.2f}  \n'
            f'No of trades: {self.account.report.n_trades or np.nan:0.2f}  \n'
            f'Trades win: {self.account.report.n_trades_win or np.nan:0.2f}  \n'
            f'trades loss: {self.account.report.n_trades_loss or np.nan:0.2f}  \n'
        )
